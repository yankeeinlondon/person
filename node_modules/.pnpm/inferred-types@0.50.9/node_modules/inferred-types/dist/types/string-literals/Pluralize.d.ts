...
import { Consonant, IsStringLiteral, EnsureTrailing, StripTrailing, Whitespace, TrimRight, Join, RightWhitespace } from "src/types/index";
 ...
 */
type PluralException<T extends string> = T extends keyof ExceptionLookup ? ExceptionLookup[T] : never;
...
type _Pluralize<T extends string> = IsException<T> extends true ? PluralException<T> : T extends EndsIn_IS<T> ? PluralizeEndingIn_IS<T> : T extends EndsInSingularNoun<T> ? PluralizeEndingSingularNoun<T> : T extends EndsIn_F<T> ? PluralizeEnding_F<T> : T extends EndsIn_Y<T> ? PluralizeEndingIn_Y<T> : `${T}s`;
/**
 * **Pluralize**`<T>`
 *
 ...
 */
export type Pluralize<T extends string> = IsStringLiteral<T> extends true ? T extends `${string}${Whitespace}` ? Join<[
