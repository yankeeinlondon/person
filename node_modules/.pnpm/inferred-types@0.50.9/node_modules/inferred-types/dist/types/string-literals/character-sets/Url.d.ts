import { AlphaNumericChar, AsString, ExpandUnion, IsStringLiteral, StripAfter, StripChars, StripLeading, StripTrailing, TupleToUnion, DnsName, DomainName, Ip4Address, Mutable, Values, Flatten, EmptyObject, IsEqual, NumericChar, RetainWhile, AsNumber, StripWhile, IsUndefined, Contains, RemoveEmpty } from "src/types/index";
import type { NETWORK_PROTOCOL_LOOKUP } from "src/constants/index";
type Proto = typeof NETWORK_PROTOCOL_LOOKUP;
export type NetworkProtocol = Mutable<Values<Proto>> extends readonly (string | string[])[] ? RemoveEmpty<Flatten<Mutable<Values<Proto>>>>[number] : never;
/**
 ...
 */
export type GetUrlQueryParams<T extends string, S extends string | undefined = undefined> = T extends `${string}?${infer QP}` ? IsUndefined<S> extends true ? `?${QP}` : Contains<QP, `${S}=`> extends true ? string : undefined : "";
/**
 * **AnyQueryParams**
 *
