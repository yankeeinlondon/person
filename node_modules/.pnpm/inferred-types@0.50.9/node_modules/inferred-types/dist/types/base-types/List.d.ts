import { AsArray, Fail, AnyFunction, IsInteger, Dictionary, Flatten } from "src/types/index";
...
export type List<T = unknown, ID extends string = string> = {
    ...
    */
     ...
     */
    every: <TCall extends ((v: T) => boolean), TReturn extends boolean>(cb: TCall) => TReturn;
    /**
     * **fill**`(val)`
     *
     ...
     */
    flat: <TDepth extends number = 1>(depth?: TDepth) => List<Flatten<T, TDepth extends 1 | 2 | 3 ? TDepth : 3, true>, `${ID} -> flattened${TDepth extends 1 ? "" : `(${TDepth})`}`>;
    /**
     ...
     * callback.
     */
    flatMap: <TCall extends ((v: T) => unknown), TArg>(cb: TCall, thisArg?: TArg) => List<Flatten<ReturnType<TCall>, 1, true>, `${ID} -> mapped`>;
    /**
     * **forEach**`(callback)`
     *
     ...
     */
    reduce: <TCall extends ((acc: T[], v: T) => unknown), TVal = List<ReturnType<TCall>, `${ID} -> reduced`>>(cb: TCall, initialValue?: TVal) => TVal;
    /**
     ...
     */
    reduceRight: <TCall extends ((acc: T[], v: T) => unknown), TVal = List<ReturnType<TCall>, `${ID} ->reducedRight`>>(cb: TCall, initialValue?: TVal) => TVal;
    /**
     * **reverse**`()`
     *
     ...
     */
    slice: <TStart extends number, TStop extends number>(start?: TStart, stop?: TStop) => ShallowCopy<List<T, `${ID}-> sliced`>>;
    /**
     * **some**`(callback)`
     *
     ...
     */
    splice: (start: number, deleteCount?: number, ...items: T[]) => List<T, `${ID} -> spliced`>;
    /**
     * **toLocaleString**`()`
     *
     ...
     */
    toLocaleString: <TLang extends BCP47, TOpts extends Dictionary>(lang: TLang, opt?: TOpts) => string;
    /**
     * **toReversed**`()`
     *
     ...
     */
    toSpliced: (start: number, deleteCount?: number, ...items: T[]) => List<T, `${ID} -> toSpliced`>;
    /**
     * **toString**`()`
     *
