import type { SIMPLE_DICT_VALUES, SIMPLE_MAP_KEYS, SIMPLE_MAP_VALUES, SIMPLE_SET_TYPES } from "src/constants/index";
import { ExpandDictionary, SimpleScalarToken, SimpleToken, SimpleUnionToken, Dictionary, AsNumber, CsvToStrUnion, CsvToTuple, CsvToUnion, UnderlyingType, TupleToUnion, SimpleDictToken, SimpleMapToken, SimpleSetToken, SimpleArrayToken } from "src/types/index";
...
export type SimpleTypeArray<T extends SimpleArrayToken> = T extends "Array" ? any[] : T extends "Array<string>" ? string[] : T extends `Array<string(${infer StringUnion})>` ? Array<SimpleTypeScalar<`string(${StringUnion})`>> : T extends "Array<number>" ? number[] : T extends `Array<number(${infer NumericUnion extends `${number}`})>` ? Array<SimpleTypeScalar<`number(${NumericUnion})`>> : T extends "Array<boolean>" ? boolean[] : T extends "Array<unknown>" ? unknown[] : never;
export type SimpleTypeMap<T extends SimpleMapToken> = T extends "Map" ? Map<any, any> : T extends `Map<${infer Key extends MapKeys}, ${infer Value extends MapValues}>` ? Map<SimpleType<Key>, SimpleType<Value>> : never;
export type SimpleTypeSet<T extends SimpleSetToken> = T extends "Set" ? Set<any> : T extends `Set<${infer Type extends SetTypes}>` ? Set<SimpleType<Type>> : never;
/**
 * **SimpleType**`<T>`
 *
