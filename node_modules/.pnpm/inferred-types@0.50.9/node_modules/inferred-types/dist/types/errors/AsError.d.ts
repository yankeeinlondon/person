...
type Props<T extends AnyObject | undefined> = T extends AnyObject ? T : EmptyObject;
type IdFrom<T extends AnyObject> = AsIndexOf<T, "id", never>;
type LibraryFrom<T extends AnyObject> = AsIndexOf<T, "library", never>;
type Process<T extends AsError__Meta> = IsEqual<T, [string, string]> extends true ? ErrorCondition<T[0], T[1], never> : T extends [string, string, TypeErrorInfo] ? Throw<T[0], T[1], "AsError", {
    id: IdFrom<Props<T[2]>>;
 ...
 * **Related:** `Throw`
 */
export type AsError<TType> = TType extends ErrorCondition ? TType : IfNever<TType, ErrorCondition<"never-value", "a 'never' type was encountered which is not allowed in this context!">, TType extends Error<string> ? Throw<"runtime-error", `the JS runtime's Error class was found with the message: '${TType["message"]}'`, "AsError", {
    library: "inferred-types";
}> : TType extends AsError__Meta ? Process<TType> : Throw<"failed-to-wrap", Concat<[
    "An unexpected value -- ",
    ToString<TType>,
