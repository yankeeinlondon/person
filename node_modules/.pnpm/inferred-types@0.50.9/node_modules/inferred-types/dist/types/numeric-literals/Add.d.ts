import { IsString, IsNegativeNumber, If, Or, And, IsWideType, As } from "../boolean-logic";
...
type Either<left extends boolean, right extends boolean> = left extends true ? true : right extends true ? true : false;
type DecrementDigit<digit extends NumericChar> = digit extends "1" ? "0" : digit extends "2" ? "1" : digit extends "3" ? "2" : digit extends "4" ? "3" : digit extends "5" ? "4" : digit extends "6" ? "5" : digit extends "7" ? "6" : digit extends "8" ? "7" : digit extends "9" ? "8" : digit extends "10" ? "9" : never;
type IncrementDigit<digit extends NumericChar> = digit extends "0" ? {
    result: "1";
    carry: false;
} : digit extends "1" ? {
    result: "2";
...
type SingleDigitSumResult<result extends NumericChar, carry extends boolean> = {
    result: result;
    carry: carry;
};
type SumSingleDigits<left extends NumericChar, right extends NumericChar, carryIn extends boolean = false, carryOut extends boolean = false> = carryIn extends true ? IncrementDigit<left> extends SingleDigitSumResult<infer leftIncremented, infer carryOutFromIncrement> ? SumSingleDigits<leftIncremented, right, false, carryOutFromIncrement> : never : right extends "0" ? {
    result: left;
    carry: carryOut;
} : IncrementDigit<left> extends SingleDigitSumResult<infer leftIncremented, infer carryOutFromIncrement> ? SumSingleDigits<leftIncremented, DecrementDigit<right>, false, Either<carryOut, carryOutFromIncrement>> : never;
type RightMostDigitResult<rest extends string, digit extends NumericChar> = {
    rest: rest;
    digit: digit;
};
type RightMostDigit<s extends string> = s extends `${infer rest}${NumericChar}` ? s extends `${rest}${infer digit}` ? {
    rest: rest;
    digit: digit;
} : never : never;
...
type PreProcess<A extends NumberLike, B extends NumberLike> = CheckWide<A, B> extends true ? Or<[IsString<A>, IsString<A>]> extends true ? string : number : Or<[IsWideType<A>, IsWideType<B>]> extends true ? If<IsString<A>, string, number> : A extends `${number}` ? B extends `${number}` ? As<Process<A, B>, `${number}`> : As<Process<A, AsString<B>>, `${number}`> : A extends number ? B extends number ? AsNumber<Process<`${A}`, `${B}`>> : AsNumber<Process<`${A}`, As<B, `${number}`>>> : never;
/**
 * **Add**`<A,B>`
 *
 ...
 */
export type Add<A extends NumberLike, B extends NumberLike> = A extends number ? As<PreProcess<A, B>, number> : A extends `${number}` ? As<PreProcess<A, B>, `${number}`> : never;
export {};
