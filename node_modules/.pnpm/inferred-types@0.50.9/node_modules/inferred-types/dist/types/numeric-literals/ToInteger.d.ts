...
type Round<T extends `${number}`> = T extends `${number}.${number}` ? T extends `${infer Q}.${infer D}${number}` ? If<Extends<D, "0" | "1" | "2" | "3" | "4">, Q & `${number}`, Increment<Q & `${number}`>> : never : T;
type EnsureKind<TKind extends number | `${number}`, TVal extends `${number}`> = TKind extends number ? ToNumber<TVal> : TVal;
/**
 * **ToInteger**`<T>`
 *
 ...
 */
export type ToInteger<TVal extends number | `${number}`, TOp extends ToIntegerOp = "truncate"> = TOp extends "truncate" ? EnsureKind<TVal, Truncate<`${TVal}` & `${number}`>> : EnsureKind<TVal, Round<`${TVal}` & `${number}`>>;
export {};
