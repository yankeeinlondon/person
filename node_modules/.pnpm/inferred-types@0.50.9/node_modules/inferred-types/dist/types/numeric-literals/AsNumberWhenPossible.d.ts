...
type Convert<T> = T extends `${number}` ? ToNumber<T> : T;
type ConvertTuple<T> = T extends unknown[] ? {
    [K in keyof T]: Convert<T[K]>;
} : never;
type ConvertUnion<T> = If<IsUnion<T>, TupleToUnion<ConvertTuple<UnionToTuple<T>>>, never>;
/**
 ...
 * still be a _union_.
 */
export type AsNumberWhenPossible<T> = If<IsUnion<T>, ConvertUnion<T>, T extends readonly unknown[] ? ConvertTuple<T> : Convert<T>>;
export {};
//# sourceMappingURL=AsNumberWhenPossible.d.ts.map