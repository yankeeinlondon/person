import { Split, AfterFirst, First, AsNumber, AnyFunction, Dictionary, IsNever, Throw, Contains, If } from "src/types/index";
import type { TT_Atomics, TT_Containers, TT_Functions, TT_SEP, TT_START, TT_STOP, TT_Singletons, TT_Sets, SIMPLE_SCALAR_TOKENS, SIMPLE_UNION_TOKENS, SIMPLE_OPT_SCALAR_TOKENS, SIMPLE_DICT_TOKENS, SIMPLE_ARRAY_TOKENS, SIMPLE_MAP_TOKENS, SIMPLE_SET_TOKENS } from "src/constants/index";
/**
 * **SimpleToken**
 *
 ...
 * - `TypeToken`
 */
export type SimpleToken = SimpleScalarToken | SimpleArrayToken | SimpleDictToken | SimpleMapToken | SimpleSetToken | SimpleUnionToken;
/**
 ...
 */
export type SimpleScalarToken = RequiredSimpleScalarTokens | OptionalSimpleScalarTokens;
/**
 * **SimpleUnionToken**
 *
 ...
 */
export type SimpleContainerToken = SimpleDictToken | SimpleArrayToken | SimpleMapToken | SimpleArrayToken;
/**
 * TypeTokenAtomics
 *
 ...
 */
export type AtomicToken<T extends TypeTokenAtomics = TypeTokenAtomics> = T extends TypeTokenAtomics ? `<<${T}>>` : never;
 ...
 */
export type SingletonToken = `<<${TypeTokenSingletons}>>` | `<<${TypeTokenSingletons}::${string}>>` | `<<${TypeTokenSingletons}::${UnionToken}>>` | `<<string-set::${string}::[${string}]>>` | `<<number-set::${string}::[${string}]>>`;
 ...
 * token identifier)
 */
export type TypeTokenKind = TypeTokenAtomics | TypeTokenContainers | TypeTokenFunctions | TypeTokenSets | TypeTokenSingletons;
/**
 ...
 *
 */
export type TypeToken<T extends TypeTokenKind = TypeTokenKind> = `${TypeTokenStart}${T}${"" | `::${string}`}${TypeTokenStop}`;
...
type Type<TCat extends Categories, TKind extends TypeTokenKind, TData extends readonly string[]> = TCat extends "atomics" ? GetAtomic<TKind> : TCat extends "singletons" ? GetSingleton<TKind, TData> : TCat extends "functions" ? GetFunction<TKind, TData> : TCat extends "containers" ? GetContainer<TKind, TData> : never;
...
export type ParseToken<T extends string> = T extends TypeToken ? {
    ...
    data: Data<T>;
} & TypeHelper<Type<Category<T>, Kind<T>, Data<T>>> : Throw<"invalid-token", `Call to ParseToken<T> with an invalid token: ${T}`, "ParseToken", {
...
export type TokenLiteralType<T extends TypeToken> = "literal" extends keyof ParseToken<T> ? ParseToken<T>["literal"] : never;
export type IsTypeToken<T> = T extends string ? T extends TypeToken ? true : false : false;
export {};
//# sourceMappingURL=TypeToken.d.ts.map