...
type Process<TValues extends readonly unknown[], TIgnore, TNotFound> = [] extends TValues ? TNotFound : First<TValues> extends TIgnore ? Process<AfterFirst<TValues>, TIgnore, TNotFound> : First<TValues>;
/**
 ...
 * by default.
 */
export type Iff<TValues extends NonArray | readonly unknown[], TIgnore = Nothing, TOffset extends PropertyKey | null = null, TNotFound = Throw<"not-found", `Call to Iff utility resulted in no valid results`, "Iff", {
    library: "inferred-types";
    values: AsArray<TValues>;
}>> = TOffset extends PropertyKey ? TValues extends readonly (Container | TIgnore)[] ? ProcessContainers<AsArray<TValues>, TIgnore, TOffset, TNotFound> : Throw<"invalid-values", `When calling Iff with a TOffset set; the TValues generic must contain a tuple of containers!`, "Iff", {
    library: "inferred-types";
    values: AsArray<TValues>;
    offset: TOffset;
}> : Process<AsArray<TValues>, TIgnore, TNotFound>;
export {};
//# sourceMappingURL=Iff.d.ts.map