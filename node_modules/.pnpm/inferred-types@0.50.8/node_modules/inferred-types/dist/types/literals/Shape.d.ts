import { If, Narrowable, ObjectKey, IsNever, IsTrue, TupleToUnion, IsFalse, Tuple, HandleDoneFn, AsLiteralFn, AsArray, ZipCode, MilitaryTime, TimeResolution, CivilianTime, Ip4Address, Ip6Address, Zip, ZipPlus4, As, Container, TypeTokenKind, Suggest, SimpleToken } from "src/types/index";
 ...
 * a tuple or union.
 */
export type ShapeTupleOrUnion<TTuple extends readonly Narrow[] = Narrow[], TMakeUnion extends boolean = boolean> = {
    add: <TAdd extends Narrow>(a: TAdd) => ShapeTupleOrUnion<[...TTuple, TAdd], TMakeUnion>;
    done: () => If<IsTrue<TMakeUnion>, TupleToUnion<TTuple>, TTuple>;
};
...
export type StringTokenUtilities<T> = {
     ...
     */
    regex: <TExp extends string | RegExp, TRep extends readonly SimpleToken[]>(re: TExp, ...representation: TRep) => unknown;
    done: () => T;
};
export type ShapeApi__Scalars<TUnion = never, TExclude extends string = ""> = Omit<{
    /**
     * **string**(_literals_)
     *
     * When called with:
     ...
     */
    string: (<T extends readonly string[]>(...literals: T) => T["length"] extends 0 ? StringTokenUtilities<string> : T["length"] extends 1 ? T[0] : TupleToUnion<T>);
    /**
     * **number**(_literals_)
     *
     * When called with:
     ...
     */
    number: <T extends readonly number[]>(...literals: T) => T["length"] extends 0 ? number : T["length"] extends 1 ? T[0] : TupleToUnion<T>;
    /**
     * **boolean**(_literal_)
     *
     ...
     */
    boolean: <L extends boolean>(v?: L) => If<IsTrue<L>, true, If<IsFalse<L>, false, boolean>>;
    /**
     * **null**()
     *
     * Set as a `null` value.
...
type ShapeApi__Functions = {
    fn: <TArgs extends readonly FnArgsDefn[]>(...args: TArgs) => ({
        returns: <TReturn extends FnReturnTypeDefn>(rtn: TReturn) => ({
            addProperties: <TProps extends FnPropertiesDefn>(kv: TProps) => AsLiteralFn<FromWideTokens<TArgs, FromDefn<TArgs>>, FromWideTokens<TReturn, FromDefn<TReturn>>, FromDefn<TProps>>;
...
export type FromRecordKeyDefn<T extends RecordKeyDefn> = T extends ShapeCallback ? HandleDoneFn<ReturnType<T>> : T extends "string" ? string : T extends "symbol" ? symbol : T extends "string | symbol" ? string | symbol : never;
/**
 ...
 * the function.
 */
export type ShapeApi = ShapeApi__Scalars & ShapeApi__Union & ShapeApi__Functions & ShapeApi__WideContainers & ShapeApi__LiteralContainers;
/**
 * **ShapeCallback**
 *
