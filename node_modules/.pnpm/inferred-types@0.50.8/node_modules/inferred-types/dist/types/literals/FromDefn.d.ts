...
import { EmptyObject, Dictionary, ObjectKey, Tuple, IsEqual, Contains, Keys, Values, HandleDoneFn, AfterFirst, First, AsDictionary, TupleToUnion, ExpandDictionary, NarrowableScalar, ShapeCallback, UnionElDefn } from "src/types/index";
type HandleObject<TObj extends Dictionary, TKeys extends readonly unknown[], TResult extends Dictionary = EmptyObject> = [] extends TKeys ? ExpandDictionary<TResult> : HandleObject<TObj, AfterFirst<TKeys>, First<TKeys> extends keyof TObj ? TObj[First<TKeys>] extends ShapeCallback ? Record<First<TKeys>, HandleDoneFn<ReturnType<TObj[First<TKeys>]>>> & TResult : Record<First<TKeys>, TObj[First<TKeys>]> & TResult : never>;
 ...
 */
export type AsUnion<T extends UnionElDefn | readonly [UnionElDefn, ...UnionElDefn[]]> = T extends UnionElDefn[] ? TupleToUnion<IterateUnion<T>> : T extends UnionElDefn ? ProcessUnion<T> : never;
export type IsDictionaryDefinition<T> = T extends Dictionary ? Contains<Values<T>, ShapeCallback> : false;
/**
 * converts non-tuple definition types to actual type
 */
type ToType<T, TElse> = [
    T
] extends [ShapeCallback] ? HandleDoneFn<ReturnType<T>> : [IsDictionaryDefinition<T>] extends [true] ? HandleObject<AsDictionary<T>, Keys<AsDictionary<T>>> : IsEqual<TElse, Constant<"not-set">> extends true ? T : TElse;
/**
 * iterates over tuple definition types to convert into real types
 */
type IterateOverDefinitions<T extends readonly unknown[], TElse> = {
 ...
 */
export type DictTypeDefinition<V extends TypeDefinition = TypeDefinition> = Record<ObjectKey, V>;
/**
 * **FromDefn**`<T, [TElse]>`
 *
 ...
 */
export type FromDefn<T, TElse = Constant<"not-set">> = T extends readonly unknown[] ? IterateOverDefinitions<T, TElse> : ToType<T, TElse>;
export {};
//# sourceMappingURL=FromDefn.d.ts.map