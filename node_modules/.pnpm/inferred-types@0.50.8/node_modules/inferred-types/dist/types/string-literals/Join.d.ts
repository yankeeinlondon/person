import { IsGreaterThan, AfterFirst, First, ToStringArray, TakeFirst } from "src/types/index";
type Process<TTuple extends readonly string[], TSeparator extends string, TResult extends string = ""> = [] extends TTuple ? TResult : Process<AfterFirst<TTuple>, TSeparator, TResult extends "" ? First<TTuple> extends "" ? TResult : `${First<TTuple>}` : First<TTuple> extends "" ? TResult : `${TResult}${TSeparator}${First<TTuple>}`>;
 ...
 * **Related:** `Concat<TArr>`
 */
export type Join<TTuple extends readonly unknown[], TSeparator extends string = "", TMax extends number | null = null, TEllipsis extends string | false = "..."> = ToStringArray<TTuple> extends readonly string[] ? TMax extends number ? IsGreaterThan<TTuple["length"], TMax> extends true ? Process<Slicer<TTuple, TMax, TEllipsis>, TSeparator> : Process<ToStringArray<TTuple>, TSeparator> : Process<ToStringArray<TTuple>, TSeparator> : never;
export {};
