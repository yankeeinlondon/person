import type { Narrowable, Box, IsArray, Extends, If, Or, IsStringLiteral, IsEqual, IsUnion, IsFalse, As } from "src/types/index";
import type { LITERAL_TYPE_KINDS, WIDE_TYPE_KINDS, NARROW_CONTAINER_TYPE_KINDS, WIDE_CONTAINER_TYPE_KINDS, FALSY_TYPE_KINDS, NoDefaultValue, NotApplicable } from "src/constants/index";
import { TupleToUnion, TypeGuard, Filter, UnionToIntersection, AnyFunction, ErrorCondition } from "src/types/index";
export type TypeOptions<TKind extends TypeKind = TypeKind, TRequired extends boolean = boolean, TDesc extends string = string, TUnderlying extends TypeUnderlying = TypeUnderlying, TDefaultValue extends TypeDefaultValue<TKind, TRequired, TUnderlying> = TypeDefaultValue<TKind, TRequired, TUnderlying>, TValidations extends readonly unknown[] | "no-validations" = readonly unknown[] | "no-validations"> = {
 ...
 */
export type IsLiteralKind<T extends TypeKind> = If<Or<[
    If<Extends<T, TypeKindLiteral>, true, false>,
    If<Extends<T, TypeKindContainerNarrow>, true, false>
]>, true, false>;
export type IfLiteralKind<T extends TypeKind, IF extends Narrowable, ELSE extends Narrowable> = IsLiteralKind<T> extends true ? IF : ELSE;
/**
 * **ToType**
 *
 ...
 */
type ToType<TKind extends TypeKind, TRequired extends boolean | TypeIsRequired = boolean | TypeIsRequired, TUnderlying extends TypeUnderlying = "no-underlying"> = If<Or<[
    Extends<TRequired, "not-required">,
    Extends<TRequired, false>
]>, ToBaseType<TKind, TUnderlying> | undefined, ToBaseType<TKind, TUnderlying>>;
type ToBaseType<TKind extends TypeKind, TUnderlying extends TypeUnderlying = "no-underlying"> = TKind extends "string" ? string : TKind extends "number" ? number : TKind extends "boolean" ? boolean : TKind extends "null" ? null : TKind extends "undefined" ? undefined : TKind extends "stringLiteral" ? TupleToUnion<TUnderlying> : TKind extends "numericLiteral" ? TupleToUnion<TUnderlying> : TKind extends "true" ? true : TKind extends "false" ? false : TKind extends "anyArray" ? unknown[] : TKind extends "unknownObject" ? Record<string, unknown> : TKind extends "unknownObject" ? Record<string, unknown> : TKind extends "unknownFunction" ? AnyFunction : TKind extends "unknownObject" ? Record<string, unknown> : TKind extends "fnWithDict" ? TUnderlying extends readonly [
    {
        kind: "fnType";
        type: unknown;
        [key: string]: unknown;
...
] ? TUnderlying[0]["type"] & TUnderlying[1]["type"] : never : TKind extends "tuple" ? TUnderlying extends readonly unknown[] ? TupleToUnion<TUnderlying> : never : TKind extends "union" ? TUnderlying extends readonly unknown[] ? TupleToUnion<Filter<TUnderlying, {
    kind: TypeKind;
    required: TypeIsRequired;
    underlying: readonly unknown[] | "none";
}>> : never : TKind extends "intersection" ? TUnderlying extends readonly unknown[] ? UnionToIntersection<TupleToUnion<Filter<TUnderlying, {
    kind: TypeKind;
    required: TypeIsRequired;
    underlying: readonly unknown[] | "none";
}>>> : never : unknown;
 ...
 */
export type TypeDefaultValue<TKind extends TypeKind = TypeKind, TRequired extends boolean = boolean, TUnderlying extends TypeUnderlying = TypeUnderlying> = Box<ToType<TKind, TRequired, TUnderlying>> | NoDefaultValue;
/**
 ...
 */
export interface TypeDefn<TKind extends TypeKind = TypeKind, TRequired extends boolean = boolean, TDesc extends string = string, TUnderlying extends TypeUnderlying = TypeUnderlying, TDefaultVal extends TypeDefaultValue<TKind, TRequired, TUnderlying> = TypeDefaultValue<TKind, TRequired, TUnderlying>, TValidations extends TypeDefnValidations = TypeDefnValidations> {
    readonly kind: TKind;
 ...
 */
export type FromTypeDefn<TD extends TypeDefn> = TD extends TypeDefn<infer Kind, infer Required, infer Desc, infer Underlying, infer DefaultValue, infer Validations> ? Type<Kind, IsEqual<Required, boolean> extends true ? "required" : IsFalse<Required> extends true ? "not-required" : "required", IsStringLiteral<Desc> extends true ? Desc : "", IsArray<Underlying> extends true ? Exclude<Underlying, "no-underlying"> : "no-underlying", As<If<Or<[
    IsEqual<DefaultValue, NoDefaultValue>,
    IsUnion<DefaultValue>
]>, "no-default-value", "with-default-value">, TypeHasDefaultValue>, As<If<Or<[
    IsEqual<Validations, "no-validations">,
    IsUnion<Validations>
]>, "no-validations", "with-validations">, TypeHasValidations>> : never;
 ...
 */
export type Type<TKind extends TypeKind = TypeKind, TRequired extends TypeIsRequired = TypeIsRequired, TDesc extends string = string, TUnderlying extends TypeUnderlying = TypeUnderlying, TWithDefault extends TypeHasDefaultValue = TypeHasDefaultValue, TWithValidations extends TypeHasValidations = TypeHasValidations> = {
    _type: "Type";
    /**
     * **kind**
     *
     ...
     */
    underlying_operand: TUnderlying extends "no-underlying" ? "NA" : TKind extends "intersection" ? "AND" : "OR";
    /**
     * **defaultValue**
     *
     ...
     */
    defaultValue: TWithDefault extends "no-default-value" ? NoDefaultValue : Box<ToType<TKind, TRequired, TUnderlying>>;
    /**
     * **isUnion**
     *
     ...
     */
    validations: TWithValidations extends "no-validations" ? readonly [] : readonly ValidationFunction[];
