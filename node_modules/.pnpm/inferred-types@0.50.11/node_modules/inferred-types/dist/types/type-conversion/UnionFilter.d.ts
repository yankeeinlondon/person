 ...
 * **Related:** `UnionFilter`
 */
export type UnionRetain<U, E> = [U] extends [never] ? never : IsUnion<U> extends true ? Isolate<UnionToTuple<U>, E> : U extends E ? U : never;
export type UnionMutationOp = "Capitalize" | "Lowercase" | "CamelCase" | "PascalCase" | "SnakeCase" | "KebabCase";
type Mutate<TElements extends readonly unknown[], TOp extends UnionMutationOp> = TOp extends "Capitalize" ? {
    ...
    [K in keyof TElements]: TElements[K] extends string ? Lowercase<TElements[K]> : TElements[K];
} : TOp extends "CamelCase" ? {
    [K in keyof TElements]: TElements[K] extends string ? CamelCase<TElements[K]> : TElements[K];
} : TOp extends "PascalCase" ? {
    [K in keyof TElements]: TElements[K] extends string ? PascalCase<TElements[K]> : TElements[K];
} : TOp extends "KebabCase" ? {
    [K in keyof TElements]: TElements[K] extends string ? KebabCase<TElements[K]> : TElements[K];
} : TOp extends "SnakeCase" ? {
    [K in keyof TElements]: TElements[K] extends string ? SnakeCase<TElements[K]> : TElements[K];
} : never;
export type UnionMutate<U, Op extends UnionMutationOp> = [U] extends [never] ? never : IsUnion<U> extends true ? Mutate<UnionToTuple<U>, Op>[number] : U;
export {};
//# sourceMappingURL=UnionFilter.d.ts.map