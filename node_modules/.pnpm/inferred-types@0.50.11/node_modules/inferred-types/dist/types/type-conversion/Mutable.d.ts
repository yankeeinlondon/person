...
type MutableObject<T> = [T] extends [boolean] ? T : {
    -readonly [K in keyof T]: T[K] extends Dictionary ? MutableObject<T[K]> : IsTuple<T[K]> extends true ? T[K] : T[K] extends readonly (infer R)[] ? R[] : T[K];
};
 ...
 * widening the type.
 */
export type Mutable<T> = [T] extends [readonly unknown[]] ? MutableArray<T> : [T] extends [boolean] ? T : [IsObjectLiteral<T>] extends [true] ? MutableObject<T> : T;
/**
 * **Immutable**`<T>`
 *
