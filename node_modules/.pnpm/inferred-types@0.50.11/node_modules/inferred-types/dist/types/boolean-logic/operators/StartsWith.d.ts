import { AsString, IsWideType, TupleToUnion, IsEqual } from "src/types/index";
type Check<TValue extends string, TComparator extends string | number> = TValue extends `${TComparator}${string}` ? true : false;
type Process<TValue extends string, TComparator extends string | number | readonly string[]> = TComparator extends readonly string[] ? Check<[
    TValue
] extends [number] ? `${TValue}` : TValue, TupleToUnion<TComparator>> : Check<[
    TValue
] extends [number] ? `${TValue}` : TValue, AsString<TComparator>>;
/**
 ...
 */
export type StartsWith<TValue extends string | number, TComparator extends string | number | readonly string[]> = [IsWideType<TValue>] extends [true] ? boolean : [IsWideType<TComparator>] extends [true] ? boolean : IsEqual<Process<AsString<TValue>, TComparator>, boolean> extends true ? true : Process<AsString<TValue>, TComparator>;
export {};
