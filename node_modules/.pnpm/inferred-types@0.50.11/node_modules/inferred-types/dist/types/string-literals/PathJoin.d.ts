import { Join, First, Last } from "src/types/index";
export type StripSlash<T extends readonly string[]> = {
    [K in keyof T]: T[K] extends `/${infer Middle}/` ? Middle : T[K] extends `/${infer Trailing}` ? Trailing : T[K] extends `${infer Leading}/` ? Leading : T[K];
};
type Process<TPath extends string, TFirst extends string, TLast extends string> = TFirst extends `/${string}` ? TLast extends `${string}/` ? `/${TPath}/` : `/${TPath}` : TLast extends `${string}/` ? `${TPath}/` : TPath;
/**
 * **PathJoin**`<T,U>`
 *
 ...
 * the type.
 */
export type PathJoin<TSegments extends readonly string[]> = StripSlash<TSegments> extends readonly string[] ? Process<Join<StripSlash<TSegments>, "/">, First<TSegments>, Last<TSegments>> : never;
export {};
//# sourceMappingURL=PathJoin.d.ts.map