...
type Process<TContent extends string, TLeading extends string> = TContent extends `${TLeading}${string}` ? TContent : `${TLeading}${TContent}`;
type IterateOver<TContent extends readonly (string | number)[], TLeading extends string | number> = {
    [K in keyof TContent]: TContent[K] extends string ? Process<TContent[K], AsStringUnion<TLeading>> : TContent[K] extends number ? AsNumber<Process<`${TContent[K]}`, AsStringUnion<TLeading>>> : never;
};
