import { Delta, IsNegativeNumber, NumberLike, AsNegativeNumber, HaveSameNumericSign, Add, Abs, ParseInt, As, IsWideType, Or, IsGreaterThanOrEqual, NegDelta } from "src/types/index";
...
type Process<A extends NumberLike, B extends NumberLike> = A extends number ? B extends number ? ConvertNumber<A, B> : ConvertNumber<A, ParseInt<B>> : A extends `${number}` ? B extends `${number}` ? ConvertString<A, B> : ConvertString<A, `${As<B, number>}`> : never;
/**
 * **Subtract**`<A,B>`
 *
 * Subtracts the value of `B` _from_ `A`.
 */
export type Subtract<A extends NumberLike, B extends NumberLike> = Or<[IsWideType<A>, IsWideType<B>]> extends true ? A extends number ? number : `${number}` : Process<A, B>;
export {};
